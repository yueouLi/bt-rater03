...
df = pd.read_csv("bt_batch_03.csv", encoding="utf-8-sig")
df.columns = [col.strip() for col in df.columns]  # 去除列名首尾空格

st.set_page_config(page_title="Backtranslation Rating - Rater 03", layout="centered")
st.title("📝 Simplification Back-Translation Evaluation - Rater 03")
...
if st.button("💾 Export Ratings to CSV"):
    pd.DataFrame(ratings).to_csv(f"bt_ratings_rater03.csv", index=False, encoding="utf-8-sig")
